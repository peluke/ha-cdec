"""Sensors for Home Assistant CDEC observations."""
...
async def async_setup_entry(hass, entry, async_add_entities) -> None:
    coordinator: CdecCoordinator = hass.data[DOMAIN][entry.entry_id]
    async_add_entities(
        [CdecObservationSensor(coordinator, entry, sensor_num) for sensor_num in coordinator.sensor_nums]
    )


class _BaseSensor(CoordinatorEntity[CdecCoordinator], SensorEntity):
